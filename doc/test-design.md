...
Issue #13 の一環として、M-MAD のテスト方針とテストケースを定義する。
**現況（2026-07-02）**: L1（純粋関数）は `tests/` に実装済み・CI（GitHub Actions）導入済み（#13）。
L2（LLM モック）のうち API 失敗系（4.4 の L2-ED/JG/RN・#52）とリトライ分類（#58・`tests/test_agent.py`）
は実装済み。討論分岐（4.1〜4.3）は設計のみ。
