...
| S2-EA-2 | `extract_annotations` | 前後にテキスト＋末尾に複数 annotations | 最後のマッチを採用 |
...
| ID | 対象 | 入力（env） | 期待 |
|---|---|---|---|
| CF-1 | `get_llm_config` | 何も設定なし | provider=openai / model=gpt-4o-mini / base_url=None / api_key=OPENAI_API_KEY 参照 |
...
| L2-ED-2 | `agent.ask` が毎回例外 | ask が raise | `count<10` で終了し、フォールバック `{"annotations": []}` を返す（#5） |
| L2-JG-1 | Judge が不正 JSON を返し続ける | ask が非 JSON | 10 回後に non-translation フォールバック（#5/#7） |
