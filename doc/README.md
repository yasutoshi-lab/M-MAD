...
| ドキュメント | 内容 |
|---|---|
| [architecture.md](architecture.md) | 設計。3 ステージ構成・エージェント・討論ラウンドの進行・データフロー・**論文（arXiv:2412.20127）との対応** |
| [mqm-glossary.md](mqm-glossary.md) | MQM 用語リファレンス（4 次元・サブカテゴリ・severity・non-translation の定義と、スコア化・結果の読み方） |
| [usage.md](usage.md) | 実行ガイド（入力準備 → Stage1 → Stage2&3 → メタ評価の end-to-end。手順書 JSON 前処理・run-level jury 含む） |
| [data-format.md](data-format.md) | 入出力の形式（入力タブ区切り・Stage 出力 JSON スキーマ（success/api_failures/provider 含む）・score 形式） |
