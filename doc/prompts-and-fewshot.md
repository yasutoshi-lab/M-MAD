...
| `code/few_shot_demos_ja.py` | `ja-en` および全 `ja-*`（ソース共有） | Japanese → English（手順書ドメイン・Issue #43） |
...
1. **完全一致ペア**（`DEMO_MODULE_BY_PAIR`）: `zh-en`→`few_shot_demos`、`en-de`→`few_shot_demos_de`、
   `he-en`→`few_shot_demos_he`、`ja-en`→`few_shot_demos_ja`
2. **ソース言語**（`DEMO_MODULE_BY_SOURCE`）: source `ja`→`few_shot_demos_ja`。ja→X 診断では
   全ターゲットに同一デモを共通適用して言語間の比較可能性を担保する（設計上の共有のため
   **warning なし**。Issue #43）
3. **ターゲット言語デフォルト**（`DEMO_MODULE_BY_TARGET`）: target `en`→`few_shot_demos`、`de`→`few_shot_demos_de`
4. **English 系デフォルト**（`DEFAULT_DEMO_MODULE` = `few_shot_demos`）

3/4 に落ちた場合（専用デモ無し）は warning を出す。このフォールバックはソース言語が一致しないため、
